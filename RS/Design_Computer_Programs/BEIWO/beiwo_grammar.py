...
Term => Factor [*/] Term | Factor
Factor => Stat | Funcall | Var | Num | [+-] [(] Exp [)] | [(] Exp [)]
Funcall => Var [(] Exp [)] | Var [(] CondList [)]  
Stat => IF [(] Conds [,] Exp [,] Exp [)]
Conds => Funcall | Cond 
CondList => Cond [,] CondList | Cond
Cond => Exp >= Exp | Exp <= Exp | Exp > Exp | Exp < Exp | Exp == Exp | [\"] Text [\"] = [\"] Text [\"] 
Var => [a-zA-Z_]+
Text => [\u4e00-\u9fa5_0-9a-zA-Z]+
Num => [-+]?[0-9]+([.][0-9]*)? """)
...
parse('Exp', 'IF((1800+500)>=1800,(1800+500)/4,(IF((1800+500)>=1200,(1800+500)/3,(1800+500)/2)))-(500-240)', G)
print(timefun)
